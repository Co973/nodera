...
public final class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle saved){
        ...
        web.setWebChromeClient(new WebChromeClient(){
            @Override public boolean onShowFileChooser(WebView view,ValueCallback<Uri[]> callback,FileChooserParams params){if(fileCallback!=null)fileCallback.onReceiveValue(null);fileCallback=callback;Intent intent=new Intent(Intent.ACTION_OPEN_DOCUMENT).setType("*/*").addCategory(Intent.CATEGORY_OPENABLE).addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION|Intent.FLAG_GRANT_PERSISTABLE_URI_PERMISSION);startActivityForResult(intent,PICK_FILE);return true;}
        });
    ...
    private void requestNearby(){
        ...
        AlertDialog dialog=new AlertDialog.Builder(this).setTitle("Nearby Nodera nodes").setAdapter(adapter,(d,index)->{
    ...
    private void showNode(){
        ...
        details+="Use Nearby on both phones once to make them Bluetooth-reachable, then add each other and compare fingerprints. Files use LAN; Bluetooth carries encrypted text. Keep the Nodera notification running to receive messages.\n\nExperimental encryption: no Noise sessions or forward secrecy yet.";
        new AlertDialog.Builder(this).setTitle("Your Nodera node").setMessage(details).setPositiveButton("Close",null).setNegativeButton("Stop and lock",(d,w)->{stopService(new Intent(this,MeshService.class));finish();}).show();
    }
    ...
    @Override protected void onActivityResult(int request,int result,Intent data){super.onActivityResult(request,result,data);
        if(request==PICK_FILE&&fileCallback!=null){Uri selected=result==RESULT_OK&&data!=null?data.getData():null;if(selected!=null)try{getContentResolver().takePersistableUriPermission(selected,Intent.FLAG_GRANT_READ_URI_PERMISSION);}catch(SecurityException ignored){}fileCallback.onReceiveValue(selected==null?null:new Uri[]{selected});fileCallback=null;}
    ...
    @Override protected void onDestroy(){if(fileCallback!=null)fileCallback.onReceiveValue(null);if(bound)unbindService(connection);web.removeJavascriptInterface("NoderaAndroid");web.destroy();background.shutdownNow();super.onDestroy();}
}
